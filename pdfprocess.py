...
def general_proc(string):                       #general classement
    ...
    if data.pedaleurs[int(nombre)]==(None,None,None,None,None,None): #pedaleur not added         ######change done
        ...
        PRENOM=string[p:]
        data.pedaleurs[int(nombre)]=(NOM,PRENOM,JEUNE,NATION,EQUIPE,int(nombre))
...
def process(page):
    if ('CLASSEMENT DES JEUNES'==page[:len('CLASSEMENT DES JEUNES')]):
        ...
        for pedaleur in entries:                     #process each rider
            point_proc(pedaleur)
    elif('CLASSEMENT PAR EQUIPES'==page[:len('CLASSEMENT PAR EQUIPES')]): #data.eq[etape][equipenom]=(data.ranke,ecart)
